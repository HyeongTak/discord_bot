...
function rice(message){
  request('http://www.gsm.hs.kr/xboard/board.php?tbnum=8', function(error, response, html){
    ...
    $(S).each(function(){
        ...
        .setTitle('오늘의 급식')
        .setColor(3447003)
        .setDescription("안녕하세요. 광주 소프트웨어 마이스터고등학교 영양사이기도 합니다. 오늘의 급식을 알려드리겠습니다.")
...
client.on("message", message => {
  ...
  if (command === "이게학교냐") {
    message.channel.send("학교이기도 합니다.");
  }
  if (command === "코드업"){
...
`+data+"```");
  ...
  if (command === "help" || command === "소환"){
    const embed = new RichEmbed()
        ...
        .setColor(3447003)
        .setDescription("안녕하세요. 광주 소프트웨어 마이스터고등학교 학생이기도 합니다. 오늘은 제 얘기만 해야 하는데요, 차마 제 얘기만 할 수 없어서 도움말을 알려 드리겠습니다.")
